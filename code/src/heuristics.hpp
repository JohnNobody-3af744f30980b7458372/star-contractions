...
int handle_small_Steiner_degrees(Graph& g) {
...
//    debug_printf("%d source %d target", e.source(),e.target());
	...
	} TIMER_END("  %s: deleted %d edges in %lg s and %d rounds\n", __func__, count, timer, rounds);
}
...
void delete_edges_shortest_path(Graph &g) {
	...
	TIMER_BEGIN {
		...
		for (Vertex v = 0; v < num_vertices(g); ++v) {
			...
			SimpleDijkstra(g, v, dist, dummy, heap,
				[&](Vertex v) {
					if (neighbor_mask[v]) {
						neighbors_to_go--;
...
auto get_solution_structure(const Graph& g) {
	...
	if (g.terminals.size() <= 1) return ret;
...
Weight dreyfus_zid(Graph& g, const std::vector<std::pair<int, int>>& structure, Out out) {
	debug_printf("\nCalling %s\n", __func__);
	Weight weight = g.saved_weight();

	if (structure.size() == 0) return weight;
...
void run_all_heuristics(Graph& g) {
	...
	PAUSE_DEBUG terminal_distance_test(g);
	run_cheap_heuristics(g);
	g.compress_graph();
...
Graph end_heu(const Graph& g, const std::vector<Vertex>& possible_vertices) {
	...
	if (tmp.terminals.size() == 1) return tmp;

	TIMER_BEGIN {
	typedef std::vector<Edge> Solution;
	std::deque< Solution > cur_queue, old_queue;

	Solution best_sol = tmp.partial_solution;
	Solution orig_sol = tmp.partial_solution;
