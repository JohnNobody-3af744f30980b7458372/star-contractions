...
struct Edge {
	...
	template < typename O > friend void zelikovsky(const Graph&, O, bool);
private:
	uintptr_t ptr;
...
struct Graph {
	...
	Weight _saved_weight = 0;
	std::vector<Edge> saved_partial_solution, partial_solution;

	std::deque<EdgeData> all_edge_data;

	Weight saved_weight() const {
		return _saved_weight;
	}
	Weight partial_solution_weight() const {
		Weight weight = saved_weight();
...
Graph::Graph(const Graph &g, Graph::copy_tag) : vertex_count(g.vertex_count),
	...
	_saved_weight(g._saved_weight),  all_edge_data(g.all_edge_data) {
	...
	for (auto e : g.saved_partial_solution)
		saved_partial_solution.push_back(Edge(&all_edge_data[e.edge_data()->edge_index]));
...
void Graph::save_orig_graph() {
	debug_printf(">>> %s\n", __func__);
	assert(saved_partial_solution.empty());

	std::vector<Edge> tmp, new_sol;

	_saved_weight = orig_graph ? orig_graph->_saved_weight : 0;
	for (auto e : partial_solution) {
		if (e.is_removed()) {
			saved_partial_solution.push_back(e);
			_saved_weight += e.weight();
	...
	saved_partial_solution.clear();
	partial_solution = tmp;

	debug_printf(">> Saved weight %d\n", saved_weight());
...
Graph Graph::get_solution() const {
	...
	assert(sol.partial_solution.empty());

	std::vector<Edge> tmp(saved_partial_solution);
	tmp.insert(tmp.end(), partial_solution.begin(), partial_solution.end());

	for_each_orig_edge(tmp, [&](Vertex u, Vertex v) {
...
void Graph::compress_graph() {
	...
	for(int i = 0; i < vertex_count; i++) {
		if (degrees[i] > 0 || (is_terminal(i) && terminals.size() == 1)) {
			forward_map[i] = j;
