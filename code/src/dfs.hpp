...
struct _dfs_visitor : boost::dfs_visitor<> {
	...
	_dfs_visitor(const Graph& g, Vertex source, Backtrack b, NonTreeEdge nt, bool btr) : b(b), nt(nt), ctx(new Context) {
		ctx->source = source;
	...
	void finish_vertex(Vertex v, const Graph&) {
		assert(ctx->pred[v] != -1);
		if (ctx->btr || ctx->pred[v] >= 0) b(v, ctx->pred[v], ctx->tree_edges[v]);
	}
};

template < typename B, typename NT >
auto DFS(const Graph& g, Vertex s, B b, NT nt, bool backtrack_from_root = false) {
	_dfs_visitor<B, NT> visitor(g, s, b, nt, backtrack_from_root);
...
struct Dummy {
	template < typename ... Args > Dummy(Args...) {}
	template < typename ... Args > void operator() (Args...) const {}
	template < typename T > Dummy operator[](const T&) const { return Dummy(); }
	Dummy operator++ (int) const { return Dummy(); }
	Dummy operator* () const { return Dummy(); }
	template < typename T > operator T() const { return T(); }
	template < typename T > void push_back(const T&) const {}
};
const Dummy dummy;

#define HEAP(heap, vertex_count, cmp) \
	const auto _cmp = [&](Vertex a, Vertex b) { return cmp; }; \
	Heap<Vertex, decltype(_cmp), std::vector<unsigned>, 4> heap{_cmp}; \
	heap.map.assign(vertex_count, heap.not_in_heap)
...
#define SimpleDijkstra(g, v, dist, pred, heap, ...) do { \
		dist.assign(g.vertex_count, std::numeric_limits<decltype(+dist[0])>::max()); \
		dist[v] = 0; \
		assert(heap.empty()); \
		heap.push(v); \
		try { \
			Dijkstra(g, dist, pred, heap, ## __VA_ARGS__); \
		} catch (EarlyTerminate) { \
