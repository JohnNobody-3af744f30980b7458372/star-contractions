...
plot-all() {(
  ...
  gnuplot /dev/stdin <<EOF
    ...
    plot 'avg-noprec-mst.txt' using 1:2 with lines title 'MST', \
         ...
         'avg-noprec-zel.txt' using 1:2 with lines title 'Zelikovsky', \
         'avg-noprec-zelm.txt' using 1:2 with lines title 'Zelikovsky-', \
         'avg-noprec-zelp.txt' using 1:2 with lines title 'Zelikovsky+'
EOF

  (
    echo "\\begin{tabular}{r|ccccccc}
      \\multicolumn{8}{c}{{\\bf Averages }} \\\\
      & MST & MST+ & MST (I) & MST+ (I) & Zel & Zel\$-\$ & Zel+ \\\\ \\hline\\hline"

    for i in {0..10}; do
      echo "$((i*10))"
      for f in avg-noprec-mst{,p}.txt avg-prec-mst{,p}.txt avg-noprec-zel{,m,p}.txt; do
        echo -n '& '
...
paste -d, ../008-pace/avg-noprec-mstp.txt avg-noprec-W2c.txt > iw-noprec.txt
paste -d, ../008-pace/avg-prec-mstp.txt avg-prec-W2c.txt > iw-prec.txt
